...
export function SidebarLayout({ children }: { children: React.ReactNode }) {
  ...
  const getActiveSection = () => {
    ...
    if (pathname === "/hardware-status") return "hardware"
    if (pathname === "/scan-sessions") return "sessions"
    if (pathname === "/model-viewer") return "viewer"
    if (pathname === "/system-settings") return "settings"
    return "control"
  }
  ...
  const handleNavigation = (path: string) => {
    if (path === "control") {
      ...
      router.push("/hardware-status")
    } else if (path === "sessions") {
      router.push("/scan-sessions")
    } else if (path === "viewer") {
  ...
  return (
    <div className="flex h-screen">
      {/* Sidebar */}
      <div
        className={`${sidebarCollapsed ? "w-16" : "w-70"} bg-neutral-900 border-r border-neutral-700 transition-all duration-300 fixed md:relative z-50 md:z-auto h-full`}
      >
        <div className="p-4">
